...
from session import DEFAULT_URL
...
async def main(target: str, verbose: bool, server_url: str) -> None:
    ...
    async with MCPAgent(server_url) as agent:
        ...
        #report = await agent.analyze_changes({"f0d2baeb76056a76341fc3400f18b6b63b1b618a"}, {"611f9861f3d962f88af293f48103322c30d3527f"})
        #report = await agent.test_apis({"f0d2baeb76056a76341fc3400f18b6b63b1b618a"}, {"611f9861f3d962f88af293f48103322c30d3527f"})
        report = await agent.test_easy_apis("4")
        # report = await agent.analyze(target, verbose=verbose)
